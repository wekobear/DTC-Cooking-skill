...
本 Skill 面向支持 Agent Skills 的通用 Agent，标识与目录名为 `dtc-cooking`，品牌名为 DTC-Cooking。优先推荐 Codex；在 Claude Code、Cursor 或其它环境中，按实际可用的浏览器、代码、MCP 与协作工具执行同一流程。`agents/openai.yaml` 仅提供 Codex 展示与默认提示，不把它作为其它 Agent 执行本流程的前提。
