# Independent Site Chef（独立站厨师）

这是一个用于快速审核 DTC / Shopify / 品牌独立站转化问题的 Codex Skill。它像厨师调理一道菜一样调理独立站：先验食材（判断数据是否可信），再尝味道、找是哪个环节走了味（定位漏斗哪里流失），最后按配方调整火候（输出有证据、有优先级、有责任归属的优化清单）。
